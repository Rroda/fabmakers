...
export async function GET() {
  try {
    ...
    console.error("Erro ao listar homologações admin:", msg);
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!action || !name) {
      return NextResponse.json({ success: false, error: "Ação e nome são obrigatórios." }, { status: 400 });
    }
    ...
    const msg = error instanceof Error ? error.message : String(error);
    console.error("Erro no admin:", msg);
    return NextResponse.json({ success: false, error: msg }, { status: 500 });
  }
}
